...
def test_unicode_request(httpbin):
    r = http('--request', u'test %s' % UNICODE, 'POST', httpbin.url + '/post')
    assert HTTP_OK in r
    assert r.json['data'] == u'test %s' % UNICODE


def test_unicode_request_verbose(httpbin):
    r = http('--verbose', '--request', u'test %s' % UNICODE,
